...
export default function Profiles() {
 ...
 useEffect(() => {
  ...
  // 사용자에게 확인을 요청
  const userConfirmed = window.confirm("프로필을 업데이트 하시겠습니까?");
  ...
  if (!userConfirmed) {
    ...
    if (newAvatarUrl) {
      ...
      imgUrl = supabase.storage.from('avarars').getPublicUrl(filePath).data.publicUrl;
    }
      ...
      const { error } = await supabase
        .from('profiles')
        .update({ username: newName, avatar_url: newAvatarUrl })
        .eq('id', userUUID);
    ...
    <div className={`w-1/3 min-w-72 max-w-64 h-96 bg-transparent  backdrop-blur-sm border rounded-3xl flex-col flex items-center justify-center mx-auto ${newName ? 'border-cyan-500':'border-gray-500'}`}>
     ...
     <div className='translate-y-28 w-1/5 mx-auto opacity-0'>
      ...
      <div className='relative pointer-events-none'>
        <Image src={NewPreview} alt="l" className={`w-28 h-28 p-1 rounded-full ring-2  mx-auto pointer-events-none ${newName ? 'ring-emerald-400 dark:ring-emerald-400':'ring-gray-500 dark:ring-gray-500'}`} width={100} height={100}/>
